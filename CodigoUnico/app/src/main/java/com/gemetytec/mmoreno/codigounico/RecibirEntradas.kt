...
class RecibirEntradas : AppCompatActivity()  {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_recibir_entradas_table)
    ...
    fun DetalleProductPadreApi(IdEntrada:String){ //(GetEntradasxRecibir)
        ...
        val getData_gem = "GetEntradaXRec?IdEntrada="

        //  "http://localhost:81/api/Entradas/GetEntradaXRec?IdEntrada=2" 192.168.100.2:81
    ...
    fun GsonDetalleProdPadre(D_Json:String){
        try {
            ...
            for (datos in Response.EntradaDetalleDTO) {
                /** llenado de listas**/
                DetalleProdPadre = ModelDetalleEntradasPadre(datos.Codigo, datos.Descripcion, datos.Cantidad.toString(), datos.CantRecibida.toString(),datos.ProductoId.toString(),datos.Id.toString(),"")
  ...
  private fun obtenerListaProductosPadre() {
        ...
        for (i in DetalleProdPadreList!!.indices) {
           // listaDetalleProdPadre!!.add(DetalleProdPadreList!![i].getCodigo(),)
            listaDetalleProdPadre!!.add(ModelDetalleEntradasPadre(DetalleProdPadreList!![i].getCodigo(),DetalleProdPadreList!![i].getDescripcion(), DetalleProdPadreList!![i].getCantidad(),DetalleProdPadreList!![i].getRecibido(),"","",""))
        ...
        try {
            Handler(Looper.getMainLooper()).postDelayed({
              ...
              /*  val adaptador: ArrayAdapter<CharSequence?> =
                ...
                adapter = listviewAdapter(listaDetalleProdPadre , applicationContext)
                ...
                listview_ProductosPadre?.setOnTouchListener(View.OnTouchListener { v, event ->
                    findViewById<View>(R.id.scrollViewProd).parent
                        .requestDisallowInterceptTouchEvent(false)
                    false
                })
                ...
                listview_ProductosPadre?.onItemClickListener = AdapterView.OnItemClickListener { parent, view, position, id ->
                   ...
                   val Str_Recibido = DetalleProdPadreList!![position].getRecibido()

                    if(Str_Cantidad.contentEquals(Str_Recibido)){

                    }else if(!Str_Cantidad.contentEquals(Str_Recibido)){
    ...
    fun ProductSeleccionado(IdProducto:String,IdDetalleEntrada:String,ProdPadre:String){
        Handler(Looper.getMainLooper()).postDelayed({
        try {
            val prodCodigosHijos = Intent(this,ValidEntrCodigosHijosProd::class.java)
    ...
    fun FinalizarRecepEntrada(view: View){

        try {

            CerrarRecepcionEntradaApi(IdEntrada_Entrada,User_Login)


        }catch (E1 : Exception){
            Timber.e("FinalizarRecepEntrada: "+ E1)
            Toasty.error(this, "FinalizarRecepEntrada: "+E1.message.toString(), Toast.LENGTH_LONG).show()
        }
    ...
    fun CerrarRecepcionEntradaApi(IdEntrada:String,Usuario : String){
        // on below line we are creating a variable for our
        // request queue and initializing it.
        val queue: RequestQueue = Volley.newRequestQueue(applicationContext)
        // on below line we are creating a variable for request
        ...
        val api_gem = "CerrarRecepcionEntrada?"
        val usuario_gem = "&Usuario="
        val idEntrad_gem ="&IdEntrada="

        //  "http://localhost:81/api/Entradas/CerrarRecepcionEntrada?IdEntrada=1&Usuario=e" 192.168.100.2:81
        ...
        val request = JsonObjectRequest(Request.Method.POST, url_gem, null, { response ->
            ...
            try {
                ...
                println("cadena valida: "+cadenaJson)
                 ...
                 if (!response.getString("RespuestaEstado").toString().trim().contentEquals("") && response.getString("RespuestaEstado").toString().trim()!="Error" ){ //

                     // actualizar
                     Toasty.success(this, "Recepción Finalizada exitosa", Toast.LENGTH_LONG).show()

                     Handler(Looper.getMainLooper()).postDelayed({

                         val FinalizaRecepEntrada = Intent(this,ListadoFoliosEntradas::class.java)
                         FinalizaRecepEntrada.putExtra("L_User",User_Login)
                         FinalizaRecepEntrada.putExtra("L_IdUser",User_Id_Login)
                         FinalizaRecepEntrada.putExtra("L_IdAlmacen",almacenLoginId_Login)
                         FinalizaRecepEntrada.putExtra("L_LastDate",lastLoginDate_Login)

                         startActivity(FinalizaRecepEntrada)

                     }, 800)

                 }else if (!response.getString("RespuestaEstado").toString().trim().contentEquals("") && response.getString("RespuestaEstado").toString().trim()=="Error"){
    ...
    fun atras(view: View){
        try {
            val AtrasMenu = Intent(this,ListadoFoliosEntradas::class.java)
