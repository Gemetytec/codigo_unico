...
class ValidEntrUbicProd : AppCompatActivity() {
    ...
    var CodHijosProductList: java.util.ArrayList<ModelCodigosProductUbicar>? = null
    var listaCodHijosProduct: java.util.ArrayList<ModelCodigosProductUbicar>? = null
    var listview_ProductosPadre : ListView? = null
    var scrollPrincp :ScrollView? = null

    private var adapter: listviewAdapterCodUbicar? = null

    var EdTxt_BarCode: EditText?= null
    var Srt_BarCode: String = ""
    var Srt_UbicacionCode: String = ""

    var Srt_Ubica :String= ""

    var indeterminateSwitch: ProgressBar? = null
    var indeterminateSwitch_Cod: ProgressBar? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_recibir_codigos_ubicar)
        ...
        listview_ProductosPadre = findViewById<ListView>(R.id.lista_CodigosUnicosUbic)
        ...
        try {
            scrollPrincp?.setOnTouchListener(View.OnTouchListener { v, event ->
                findViewById<View>(R.id.lista_CodigosUnicosUbic).parent
        ...
        try {
            EdTxt_BarCode!!.setOnKeyListener(View.OnKeyListener { v, keyCode, event ->
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_UP) {
                   ...
                   ValidacionUbicacionApi(Srt_BarCode)
        ...
        val request = JsonObjectRequest(Request.Method.GET, url_gem, null, { response ->
            ...
            try {
               ...
               // println("datos_json_volley: "+ cadenaJson+ " - - ")
                ...
                if (response.getString("RespuestaEstado").toString().trim() != "false"  ){ //
                ...
                Timber.e("datos_json_volley: "+e.message.toString())
            }

        }, { error ->
            // this method is called when we get
            // any error while fetching data from our API
            Timber.e("RESPONSE IS: "+error.message.toString())
    ...
    fun GsonProdPadre_Hijos(D_Json:String){
        try {
            ...
            var CodHijosProduct: ModelCodigosProductUbicar?
            CodHijosProductList = java.util.ArrayList<ModelCodigosProductUbicar>()

            //   val Response = Gson().fromJson(D_Json, DetalleProductosPadre::class.java)

            val Response = Gson().fromJson(D_Json,CodigosEntradaAUbicarItem::class.java)

            println(Response.IdEntradaDetalle.toString() + "- - ")

            for (datos in Response.CodigosUnicos) {
                println(datos.CodigoBarras + " => " + datos.NoReimpresion+" - "+ datos.TipoMovimiento+" - ")
                /** llenado de listas**/
                CodHijosProduct = ModelCodigosProductUbicar(datos.CodigoBarras, datos.NoReimpresion.toString(), datos.TipoMovimiento)
                CodHijosProductList!!.add(CodHijosProduct)
            }
            ListViewProductUbicar()
            obtenerListaProductUbicar()

        }catch (E:Exception){
            Timber.e("GsonProdPadre_Hijos: "+E.message.toString())
        }
    }

    private fun obtenerListaProductUbicar() {
        listaCodHijosProduct = java.util.ArrayList()

        for (i in CodHijosProductList!!.indices) {
            listaCodHijosProduct!!.add(ModelCodigosProductUbicar(CodHijosProductList!![i].getCodigoBrras(),CodHijosProductList!![i].getNoImpresion(), CodHijosProductList!![i].getEstatus()))
        }
    }

    @SuppressLint("ClickableViewAccessibility")
    fun ListViewProductUbicar(){
        try {

            Handler(Looper.getMainLooper()).postDelayed({
                /**===> funcion para el llenado de la lista **/

                adapter = listviewAdapterCodUbicar(listaCodHijosProduct , applicationContext)

                println("+ ListViewCodigosUbicar +")
            ...
            Timber.e("ListViewProductosPadre: "+E.message.toString())
        }
    ...
    fun ValidacionUbicacionApi(CodUbicacion:String){
        // on below line we are creating a variable for our
        // request queue and initializing it.
        val queue: RequestQueue = Volley.newRequestQueue(applicationContext)
        // on below line we are creating a variable for request
        ...
        val api_gem = "ValidarUbicacion?"
        val cod_gem = "Ubicacion="

        //  "http://localhost:81/api/Entradas/ValidarUbicacion?Ubicacion=BB-02-01-B"  // 192.168.100.2:81

        val url_gem = endpoint_gem + method_gem + api_gem + cod_gem  + CodUbicacion
        println("url_api: "+url_gem)
        ...
        val request = JsonObjectRequest(Request.Method.POST, url_gem, null, { response ->
            ...
            try {
                ...
                // and setting it in variables.
                val cadenaJson: String = response.toString()
                println("Ubicacion valida: "+cadenaJson)

                if (!response.getString("UbicacionCorrecta").toString().trim().contentEquals("") && response.getString("UbicacionCorrecta").toString().trim()=="true" ){

                    val cadenaJson: String = response.toString()
                    println("cadena valida: "+cadenaJson)
                    Toasty.success(this, "Ubicación Reconocida" , Toast.LENGTH_SHORT).show()
                    Srt_UbicacionCode = CodUbicacion
                    indeterminateSwitch!!.visibility = View.INVISIBLE
                    EdTxt_BarCode?.getText()!!.clear()
                    UbicarCodigos(Srt_UbicacionCode)
                   // CodHijProductsApi(IdProducto_Producto,IdDetalleEntrada_Producto)

                }else if (!response.getString("UbicacionCorrecta").toString().trim().contentEquals("")&& response.getString("UbicacionCorrecta").toString().trim()=="false"){
                    indeterminateSwitch!!.visibility = View.INVISIBLE
                    EdTxt_BarCode?.getText()!!.clear()
                    Srt_BarCode=""
                    Toasty.error(this, "error en codigo de ubicación" , Toast.LENGTH_LONG).show()
                    CodHijProductsApi(IdProducto_Producto,IdDetalleEntrada_Producto)
                }else{
                    indeterminateSwitch!!.visibility = View.INVISIBLE
                    EdTxt_BarCode?.getText()!!.clear()
                ...
                // on below line we are handling our exception.
                Timber.e("ValidacionUbicacionApi_volley: "+e.message.toString())
    ...
    fun UbicarCodigos(UbicacionS:String){

        try {
            Srt_Ubica=UbicacionS
            builder = AlertDialog.Builder(this)
            val factory = LayoutInflater.from(this)
            var TextEntryView: View? = null
                TextEntryView = factory.inflate(R.layout.activity_recibir_cod_ubicacion, null)

            println("codig_a_ubicion: "+UbicacionS)
          ...
          /*  val lp = WindowManager.LayoutParams()
            ...
            // get the top most window of the android screen using getWindow() method and set the gravity of the window to
            // top that is alert dialog will be now at the topmost position
            alert!!.getWindow()?.setGravity(Gravity.TOP)

           val TxtUbicacion = TextEntryView.findViewById<View>(R.id.txt_Ubicacion) as TextView

            TxtUbicacion.setText(UbicacionS)

           val CodigoUnicoAubicar = TextEntryView.findViewById<View>(R.id.EdTxt_Codigo) as EditText

           // indeterminateSwitch_Cod = findViewById(R.id.indeterminate_circular_indicator_cod) as ProgressBar




            /** Enter de la caja de texto Calle2_edttxt **/
            CodigoUnicoAubicar.setOnKeyListener(View.OnKeyListener { v, keyCode, event ->
                if (keyCode == KeyEvent.KEYCODE_ENTER && event.action == KeyEvent.ACTION_UP) {

                    if (CodigoUnicoAubicar.getText().toString().trim().contentEquals("")) {// .equalsIgnoreCase("")
                        CodigoUnicoAubicar.setError("Codigo Sin Escanear");
                    }else{
                        indeterminateSwitch!!.visibility = View.VISIBLE

                       val String_Cod = CodigoUnicoAubicar!!.editableText.toString()
                        ...
                        ValidarEscaneoUbicarEntradaApi(String_Cod,UbicacionS)

                        Handler(Looper.getMainLooper()).postDelayed({
                            CodigoUnicoAubicar.getText()!!.clear()
                        }, 200)
            ...
            Timber.e("UbicarCodigos: "+E.message.toString())
        }
    ...
    /** CONSULTA API RecibirECU **/
    fun ValidarEscaneoUbicarEntradaApi(CodigoUnico : String,UbicacionS:String){
        // on below line we are creating a variable for our
        // request queue and initializing it.
        val queue: RequestQueue = Volley.newRequestQueue(applicationContext)
        // on below line we are creating a variable for request
        ...
        val api_gem = "ValidarEscaneoUbicarEntrada?"
        val cod_gem = "CodigoUnico="
        val ubi_gem = "&UbicacionS="
        val user_gem = "&Usuario="

        //  "http://localhost:81/api/Entradas/ValidarEscaneoUbicarEntrada?CodigoUnico=312312&UbicacionS=BB-02-01-B&Usuario=1"  // 192.168.100.2:81

        val url_gem = endpoint_gem + method_gem + api_gem + cod_gem  + CodigoUnico + ubi_gem + UbicacionS + user_gem + User_Id_Login
        println("url_api: "+url_gem)
        ...
        val request = JsonObjectRequest(Request.Method.POST, url_gem, null, { response ->
            ...
            try {
                ...
                // and setting it in variables.
                val cadenaJson: String = response.toString()
                println("ValidarEscaneoUbicarEntrada Code : "+cadenaJson)

                if (!response.getString("RespuestaEstado").toString().trim().contentEquals("") && response.getString("RespuestaEstado").toString().trim()=="true" ){
                    ...
                    Toasty.error(this, response.getString("Mensaje").toString() , Toast.LENGTH_LONG).show()
                ...
                // on below line we are handling our exception.
                Timber.e("ValidarEscaneoUbicarEntradaApi_volley: "+e.message.toString())
                //  errorUser(nombreUsuario,PassUsuario,Almacen)
            }
    ...
    fun atras(view: View){
        try {
            val AtrasRecepcionEntradas = Intent(this,UbicarEntradas::class.java)
